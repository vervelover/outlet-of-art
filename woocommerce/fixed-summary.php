...
function ap_custom_woocommerce_product_description_tab() {
	...
	// 	echo '<div class="option-heading"><h2>';
	_e('Additional Info', 'business-pro');
	echo '</h2><div class="arrow-up">-</div><div class="arrow-down">+</div></div><div class="option-content">';
	...
	?>
	        ...
	        <div class="option-content">
	            <div id="single-product-description">
	                <p>For online purchases, <?php bloginfo('name');
	?> will refund the purchase price of merchandise that is returned in its original condition and accompanied by the original invoice, original <?php bloginfo('name');
	?> packaging and security return/exchange label intact and attached to the item. Customized products can be returned with a 20€ penalty.</p>
	                <p>You must inform <?php bloginfo('name');
	?> of your intention to return the merchandise within 14 days of the date of delivery by (i) directly returning the merchandise to <?php bloginfo('name');
	?>;
	 or (ii) writing your intention to info@maxlemari.com
